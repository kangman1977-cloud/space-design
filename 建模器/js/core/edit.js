...
export function slideEdges(mesh, hes, amount, opt = {}) {
  ...
  if (bad.length) {
    /**
     * 🔴🔴 **⛔ 這句話 ⛔ 不可以寫成「要選一整圈」** —— 那是錯的。
     *
     * 【實證 2026-08-28，kang 實測第 8 項照出來的】**判準是端點的價數，
     * ⛔ 不是選了幾條邊**：
     *
     * | 選什麼 | 端點價數 | 排除選到的之後 | |
     * |---|---|---|---|
     * | **方塊原本的一條邊** | 3 價 | 剩 **2** 條 | ✅ **滑得動**（做出楔形／斜面）|
     * | 環切線上的一條邊 | 4 價 | 剩 **3** 條 | 擋下來 |
     *
     * ⭐ **單條邊滑得動是 Blender Edge Slide 本來就有的用法**，
     * kang 實測後的原話：「**效果我覺得不錯**」——⛔ 它是功能，不是漏洞。
     * ⚠ 舊訊息寫「要選一整圈」會讓使用者**放棄一個做得到的操作**
     * 〔坑第 34 條：⛔ 不要指一條不存在的退路 —— 這次是反過來，
     * 　**把一條走得通的路講成走不通**〕。
     */
    const n = bad[0].rails.length;
    return {
      ok: false,
      reason: `有 ${bad.length} 個點的旁邊有 ${n} 條邊（要剛好 2 條才知道沿哪一條走）——`
...
/**
 * ⛔ **`planeFromTwoRays()` 已於 2026-08-26 刪除（含它的 6 項測試）。**
 *
 * 它是刀具**第一版**的核心：在螢幕上點兩個位置，那條線往螢幕深處延伸
 * 成一片刀片。kang 2026-08-25 實測否決了那個設計 ——
 * 「**測試後也不知道要如何點兩點呈現我想要的切一刀位置**」，
 * 因為「往深處延伸」的那個方向使用者看不見。
 *
 * 🔴 **不是算錯，是結果唯一但事先看不到**（坑第 21 條的親戚）。
 * 現在的刀具改成「點一串**模型上的位置**」（`knifePath()`），
 * 位置記在模型上，轉視角完全不影響 —— **兩點定平面沒有已知用途了。**
 *
 * ⛔ **不要放著假裝還有人在用**（坑第 34 條：不要留一個不存在的退路）。
 * 真的要回頭做，去 `HISTORY.md` grep `planeFromTwoRays` 看當初的推導。
 */
...
export function knifePath(mesh, picks) {
  ...
  for (const k of picks) {
    ...
    /**
     * 🔴🔴 **同一條邊上已經有一個【幾乎同位置】的插入點 → 重用它，
     * ⛔ 不要再插第二個。**
     *
     * ── 【實證 2026-09-01，kang 回報「切一圈之後分離被擋」】────────
     * 閉合（快點兩下）的做法是**把第一個點複製一份加到最後**，
     * 而這裡照著在**同一個位置又插了一個新頂點** ——
     * 於是那一圈的頭尾是**兩個重合但不同的點**，中間夾著
     * **一條長度 0 的邊**。
     *
     * ⚠ **而 `分離` 是「把選到的邊當牆、看走不走得過去」** ——
     * 那條長度 0 的邊⛔ 不在牆裡，走訪就從它**穿過去**，
     * 於是上下兩半被判定成同一塊。
     *
     * 🔴 **對照組（同一個方塊、同一個位置，2026-09-01 實測）**：
     * | | 頂點 | 長度 0 的邊 | 分離 |
     * |---|---|---|---|
     * | `切一刀`（另一條程式）| 12 | 0 | ✅ 分成 2 塊 |
     * | `刀具` 閉合（修之前）| **13** | **1** | ❌ 被擋 |
     * ⭐ 兩邊的新邊都是 4 條、面數都是 10 —— **唯一的差別就是那個重複點**。
     *
     * ── ⚠ 為什麼判準用 `PLANAR_TOL_CM` ────────────────────
     * 上面那道「太靠近邊的端點」用的就是它（0.1mm）——
     * **比它近就是會長出長度 0 的線**，兩處同一個標準，⛔ 不新發明。
     *
     * ⭐ **這一段治的是【重複插點】本身，⛔ 不只是閉合那一條路** ——
     * 使用者在同一個位置點兩下、或路徑自己交錯回同一點，都一起擋掉。
     * ⚠ **⛔ 沒有動共用的 `rebuildWithInserts()`** —— 它的另一個呼叫者
     * （`邊上加點`）自己已經擋掉過短的分段，⛔ 不需要也⛔ 不該被這件事影響。
     */
    let id = -1;
    for (let i = 0; i < rec.ids.length; i++) {
      if (Math.abs(rec.ss[i] - sAB) * len < PLANAR_TOL_CM) { id = rec.ids[i]; break; }
    }
    if (id < 0) {
      id = points.length;
