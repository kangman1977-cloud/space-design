 ...
 * 材料表。
 *
 * ── 🔴 K 因子已經退出展開圖（2026-08-23）─────────────
 * 折彎時外側被拉長、內側被壓縮，中間有一層長度不變，叫中性層。
 * K 就是中性層落在板厚的幾成（從內側量起）。
 *
 * ⛔ **但它已經不影響展開圖上的任何一個數字，也不准再影響。**
 * kang 2026-08-23：「K 因子…這都是造成混亂的條件…
 * **不應該在真實尺寸中出現**」。
 *
 * 拿掉的東西：`flatten.js` 的 `ri`（圖上標的內側 R，改成標網格半徑）、
 * `sheet.js` 標題欄的 `K 0.5`、`dxf.js` 標題的 ` K0.5`、
 * 展開面板的 K 因子輸入框與「用建議值」按鈕。
 *
 * ⚠ **實測**：K 從頭到尾沒有參與圓柱的建模 —— 換 K 網格半徑一動也不動，
 * 但圖上卻印 R24.9 而網格是 25.0。那個數字描述的是不存在的東西。
 *
 * 底下各材料的 `k` 值**留著沒刪**，因為折板參數面板還在用
 * （那裡的 K 真的參與建模，換 K 連網格都變 —— 跟 seg 同類）。
 * ⚠ 但那是 `prim.js` 折板自己的 `src.k`，不是這裡的。
 * 這裡這幾個值目前**沒有任何東西在讀**，留著是為了日後做補償層時當起點。
 *
 * ── 最小折邊哪來的 ──────────────────────────────────
 * ── 最小折邊哪來的 ──────────────────────────────────
  ...
  return {
    ...
    validate(piece) {
      ...
      for (const b of (piece.bends || [])) {
        /**
         * 〔2026-08-23〕原本讀 `b.ri`（K 推出來的內側 R），那個欄位已經
         * 不存在了。改用 `b.r` ＝ **網格量出來的半徑**。
         *
         * 這裡是**檢查**（會不會裂、折邊夠不夠），不是尺寸 ——
         * 材料在這一層還是要說話，那正是它該待的地方。
         * ⚠ 但判斷的依據換成網格事實了，不再是 K 因子推出來的數字。
         */
        const ri = b.r ?? 0;
