...
export function makeRule(key, t = 0.2, override = {}) {
  ...
  return {
    ...
    /**
     * 這個材料捲得起來嗎？—— 決定展開長度走**弧長**還是**弦長**。
     *
     * ── 為什麼這是材料問題，不是幾何問題 ────────────────
     * 網格上的「圓弧」是一段一段的平面小面。這些面到底代表
     * 「一個真的被捲圓的曲面」還是「一圈平板拼出來的多邊形」，
     * **幾何本身分不出來**（踩過的坑第 10 條講的就是這件事）。
     * 分得出來的是材料：
     *
     *   紙、帆布   捲得起來 → 那些小面是離散化的產物，真實長度是弧長
     *   板材       捲不起來 → 那些小面**就是實際要切的平板**，長度就是弦長
     *
     * ── 這條錯了會怎樣（實測）──────────────────────────
     * 拿板材做角柱，程式會把弦長拉成弧長，**往多的錯**：
     *   4 角柱 +11.07%、6 角柱 +4.72%、8 段圓柱 +2.62%
     * 4 角柱會叫你切 188.50，實際只要 169.71。
     *
     * kang 2026-08-22 說明：他們用紙材、發泡板、珍珠板、木板、壓克力，
     * 「像摺紙一樣把展開圖組合起來」。板材是切開再接的，捲不起來。
     */
