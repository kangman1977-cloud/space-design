...
export class Selection {
  ...
  pickEdgePoint(clientX, clientY, snapMid = false) {
    ...
    /**
     * 🔴🔴 **吸到角：游標夠靠近那條邊的某一端 → 回傳【那個既有的頂點】。**
     *（2026-09-01；🔴 **kang 收回他自己「不做」那條拍板之後才做的**）
     *
     * ── ⚠ 這一段有一個⛔ 不可以踩的地雷，看清楚再改 ──────────
     * 🔴 **⛔ 絕對不是把 `s` 設成 0 或 1**（那看起來跟 `吸中點` 一模一樣，
     * 而 2026-09-01 就是那樣做、當天整輪撤掉的）——
     * 那會在角的位置**插一個新點**，跟原本的角重合、夾出一條**長度 0 的邊**，
     * 結果是**每一次都切不下去**。⚠ **⛔ 不是不夠準，是不能用。**
     *
     * ⭐ **正解是回一個 `v`（既有頂點的索引），⛔ 不給 `a/b/p`** ——
     * `knifePath()` 看到 `v` 就**直接重用那個頂點**，⛔ 完全不插點。
     *
     * ── 🔴 `吸中點` 開著就⛔ 不吸角（kang 2026-09-01 拍板）──────
     * ⭐ **他的理由**：`吸中點` 是使用者**明講「我現在要中點」**，⛔ 不是建議 ——
     * 這時候再把它拉到角上，就是**安靜地改掉使用者要的東西**（坑第 11 條）。
     * ⚠ 而且兩者**⛔ 永遠不會在同一個位置打架**（中點在正中間、角在兩端），
     * 所以只有「**哪一條規則生效**」要定，⛔ 沒有距離要比。
     *
     * ── ⚠ 比的是【螢幕距離】，⛔ 不是世界座標 ──────────────
     * 沿用 `EDGE_GRAB_PX` / `EDGE_GRAB_PX_TOUCH` 那一對，⛔ 不新定一個值
     * —— 跟「切點 hover 放大」同一個範圍，所以**看得到就吸得到**。
     */
    if (!snapMid) {
      const r = this._toCanvasPx(clientX, clientY);
      const grab = isTouch() ? EDGE_GRAB_PX_TOUCH : EDGE_GRAB_PX;
      if (r.w && r.h) {
        const cam = this.view.camera;
        const px = q => {
          const v = node.localToWorld(q.clone()).project(cam);
          return v.z > 1 ? null                       // 在鏡頭後面 → ⛔ 不算
            : { x: (v.x + 1) / 2 * r.w, y: (-v.y + 1) / 2 * r.h };
        };
        let bestHe = null, bd = grab;
        for (const end of [near.he.v, near.he.to]) {
          const s2 = px(end.p);
          if (!s2) continue;
          const d = Math.hypot(s2.x - r.x, s2.y - r.y);
          if (d < bd) { bd = d; bestHe = end; }
        }
        if (bestHe) {
          const p0 = bestHe.p.clone();
          return {
            obj,
            v: vi.get(bestHe.id),
            p: p0,
            world: node.localToWorld(p0.clone())
          };
        }
      }
    }
