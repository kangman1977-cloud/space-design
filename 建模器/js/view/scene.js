...
export class SceneView {
   ...
   * 給了權重就**逐點上色**：**0 ＝ 壓暗的灰**（完全不會跟著動），
   * 1 ＝ 亮橘（就是被拉的那些）。中間照權重混色，
   * 所以**顏色的濃淡直接就是「這個點會跟著動多少」**。
   *
   * 🔴🔴 **不受影響的點要【壓暗】，⛔ 不可以留白**（kang 2026-09-02 實測抓到）。
   * ⚠ 第一版是「白 → 橘」，而**白點本來就滿天都是**（一顆球 482 個），
   * 橘點夾在裡面**一眼認不出範圍**，等於這個回饋沒有作用。
   * ⭐ 改成「灰 → 橘」之後，亮起來的**就是**會動的那些 ——
   * 靠的是**明暗**而不是色相，⛔ 色弱也分得出來。
   *
   * ⚠ **⛔ 沒選東西或半徑 0 時不走這條路**（`weights` 是 `null`）——
   * 那時候全部維持原本的白，因為「顯示點」本來的意思是「這裡有一個點」。
   *
  ...
  setVertexDots(worldPts, weights = null) {
    ...
    if (useW) {
      /**
       * 暗灰 (0.30, 0.34, 0.40) → 亮橘 (1, 0.47, 0.10)。
       * ⚠ 灰刻意帶一點藍：模型是藍的，**中性灰貼在藍面上會變成髒白**。
       * ⚠ 亮度差要夠大（灰約 0.34、橘約 0.63）——
       * ⭐ **判讀靠的是亮度⛔ 不是色相**，這樣色弱也分得出來。
       *
       * ── 🔴🔴 `FLOOR`：⛔ 顏色⛔ 不是直接拿權重去混（kang 2026-09-02 第二次抓到）
       * 第二版做成「顏色濃淡 ＝ 權重」，而 kang 說「**橘色不夠明顯**」。
       * ⚠ 病因是**權重掉得比人眼需要的快**：球上格距約 5.9 cm、半徑 15 時
       * 只有三圈點 —— 中心 1.00、第一圈 0.66、**第二圈 0.11**。
       * ⇒ 最外圈幾乎全灰，**範圍的邊界等於沒畫出來**。
       *
       * ⭐ **改成「有被影響就先給 `FLOOR` 的橘，剩下才照權重分」**：
       * 　`t = FLOOR + (1 - FLOOR) × w`
       * ⇒ 最外圈也看得見，而濃淡照樣分得出誰動得多。
       *
       * 🔴 **這是一個刻意的取捨，⛔ 不要「修正」回線性**：
       * **這個顏色的工作是回答「範圍到哪」，⛔ 不是讓人讀出權重的精確值。**
       * 〔要讀數量看提示那一句「會動到 N 個點」 —— 那個數字是精確的〕
       */
      const G = [0.30, 0.34, 0.40], O = [1.00, 0.47, 0.10], FLOOR = 0.40;
      const col = new Float32Array(worldPts.length * 3);
      for (let i = 0; i < worldPts.length; i++) {
        const w = Math.max(0, Math.min(1, Number(weights[i]) || 0));
        const t = w > 0 ? FLOOR + (1 - FLOOR) * w : 0;
        col[i * 3]     = G[0] + (O[0] - G[0]) * t;
        col[i * 3 + 1] = G[1] + (O[1] - G[1]) * t;
        col[i * 3 + 2] = G[2] + (O[2] - G[2]) * t;
      }
