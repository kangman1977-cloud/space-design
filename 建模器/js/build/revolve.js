 ...
 *
 * ── 🔴🔴 介面 2026-09-02【收起來了】，⛔ 而這一支還在 ───────
 * **⛔ 這一支一行都沒刪，22 項測試也全部還在跑**（體積／χ／極點／環／
 * smooth／擋關都驗過）。收起來的是 `index.html` 那一組控制項與
 * `main.js` 的接線 —— **輪廓從哪來、中心線怎麼指定，那一半會換掉。**
 *
 * 🔴 **為什麼收**（kang 2026-09-02 一句話問到根本）：
 * > 「**你只是為了達成旋轉成形，並不是真正的計畫**」
 *
 * 整條路（去鋼筆挖線 → 挖出問題 → 再補一個輪廓物件 → 中心線看不到 →
 * 再拿參考線補）**每一步都是為了讓那顆按鈕能動**，
 * ⛔ 沒有一步是問「這個建模器缺什麼」。
 * ⇒ 真正缺的是**草圖**那一層 —— 見 `草圖與造型-設計方向.md`。
 *
 * ⭐ **而這一支之所以能原封不動留下來，正是四動作框架的功勞**：
 * 它**只吃一串點**，⛔ 不認識鋼筆、⛔ 不認識選取、⛔ 不認識文件。
 * **誰餵給它都行** —— 所以換輸入⛔ 不用重寫幾何。
 *
